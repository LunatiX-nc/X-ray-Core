...
start_xray() {
  if [[ -f '/etc/systemd/system/vmejs.service' ]]; then
...
remove_xray() {
  if systemctl list-unit-files | grep -qw 'xray'; then
    ...
    local delete_files=('/usr/local/bin/xray' '/etc/systemd/system/vmejs.service' '/etc/systemd/system/vmejs@.service' '/etc/systemd/system/vmejs.service.d' '/etc/systemd/system/vmejs@.service.d')
...
main() {
  ...
  if [[ "$CONFIG_NEW" -eq '1' ]]; then
    echo "installed: /etc/xray/vme.json"
  fi
  ...
  if [[ "$LOG" -eq '1' ]]; then
    ...
    echo 'installed: /var/log/xray/accessvme.log'
    echo 'installed: /var/log/xray/errorvme.log'
  fi
  ...
  if [[ "$XRAY_IS_INSTALLED_BEFORE_RUNNING_SCRIPT" -eq '1' ]] && [[ "$FORCE" -eq '0' ]] && [[ "$REINSTALL" -eq '0' ]]; then
    ...
    systemctl start vmejs
    systemctl enable vmejs
    sleep 1s
    if systemctl -q is-active vmejs && systemctl -q is-active xray; then
      echo -e "\e[92;1m[ info ]\e[0m: \e[92;1mEnable and start the Xray service \e[0m"
    else
      echo -e "\e[91;1mwarning\e[0m: \e[97;1m KONTOL MEMEK \e[0mFailed to enable and start the Xray service"
    fi
  fi
}
